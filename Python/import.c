...
/* MorphOS */                                                                                     
    ...
    if (NULL != FindVar("PYTHONCASEOK", GVF_BINARY_VAR))
        return 1;                                                                                 
                                                                                                  
    lock = Lock(buf, ACCESS_READ);                                                                
    if (lock != NULL) {
        result = NameFromLock(lock, tmpbuf, FILENAME_MAX+1); /* Return an absolute fullpath */
        if (result && !strncmp(FilePart(tmpbuf), name, namelen))
            result = 1;
        else
            result = 0;
