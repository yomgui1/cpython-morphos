...
size_t PyMorphOS_GetFullPath( const char *path, char *buffer, size_t size )
{
    BPTR lock = NULL;
    LONG result = FALSE;
...
size_t PyMorphOS_GetFullPathWide( const wchar_t *path, wchar_t *buffer, size_t size )
{
    ...
    if (size > 0) {
        ...
        if (!path || !wcslen(path)) {
            if (GetCurrentDirName(tmp, size))
                result = mbstowcs(buffer, tmp, size) > 0;
        } else {
            char *path2 = _Py_wchar2char(path, NULL);
            if (path2) {
                BPTR lock;
...
int PyMorphOS_AddTermFunc( void (*func)(void), const char *name )
{
    pTermFuncNode_t node;
...
*/
void *PyMorphOS_InitThread(void)
{
    pPyMorphOS_ThreadData_t td;
